...
export function scoreSlice(report) {
    const wrScore = scoreWinRate(report.overview.winRate);
    const pfScore = scorePF(report.overview.profitFactor);
    const ddScore = scoreDD(report.risk.maxDrawdown / Math.max(1, Math.abs(report.overview.totalPnL)));
