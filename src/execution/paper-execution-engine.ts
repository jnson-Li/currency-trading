// src/execution/paper-execution-engine.ts
...
export class PaperExecutionEngine implements ExecutionEngine {
    private cfg: Required<Omit<PaperExecutionConfig, 'onResult'>> & {
        onResult?: PaperExecutionConfig['onResult']
    }

    constructor(cfg?: PaperExecutionConfig) {
        this.cfg = {
            ...defaultCfg,
            ...cfg,
    ...
    private calcQty(signal: TradeSignalBase): number {
    ...
    private finish(
        ...
        // 统一补充 latencyMs（如果没填）
        if (res.latencyMs == null) res.latencyMs = Date.now() - startedAt
...
function makeSignalId(signal: TradeSignalBase, ctx: StrategyContext): string {
...
function simulateMarketFill(input: {
    ...
    const withSpread =
        side === 'long' ? marketPrice * (1 + halfSpread) : marketPrice * (1 - halfSpread)
