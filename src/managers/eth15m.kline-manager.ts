...
export class ETH15mKlineManager extends BaseKlineManager {
    ...
    private updateTrend() {
        ...
        if (last.close > upper) this.trend = 'bull'
        else if (last.close < lower) this.trend = 'bear'
    ...
    private updateStructure() {
        ...
        if (highs.length > 0) this.lastSwingHigh = highs[highs.length - 1]
        if (lows.length > 0) this.lastSwingLow = lows[lows.length - 1]
        ...
        if (h2 > h1 && l2 > l1) next = 'hh_hl'
        else if (h2 < h1 && l2 < l1) next = 'lh_ll'
        else next = 'range'
