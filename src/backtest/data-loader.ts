...
function getCacheFile(symbol: string, interval: string, startTime: number, endTime: number) {
 ...
 */
export async function loadHistorical5m(
    symbol: string,
    startTime: number,
    endTime: number
): Promise<Kline[]> {
    ...
    // 1️⃣ 先读缓存
    const cached = readJsonCache<Kline[]>(cacheFile)
    ...
    while (cursor < endTime) {
        const raws: BinanceRawKline[] = await fetchBiAnKline({
            symbol,
            interval,
            limit,
            startTime: cursor,
