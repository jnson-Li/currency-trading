...
import { EXEC_REASON, type ExecRejectReason } from '@/execution/execution-reject-reasons.js'

export function evaluateSystemHealth(snapshot: ExecutionMetricsSnapshot): SystemHealthReport {
    let score = 100
    const warnings: string[] = []
    const lifesavers: SystemHealthReport['lifesavers'] = []
    ...
    const saverReasons: Array<{ key: ExecRejectReason; note: string }> = [
    ...
    const status: SystemHealthReport['status'] =
        score < 40 ? 'danger' : score < 70 ? 'warning' : 'healthy'
